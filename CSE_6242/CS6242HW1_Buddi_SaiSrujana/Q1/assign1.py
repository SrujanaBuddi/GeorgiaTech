...
def getFollowers(api, root_user, no_of_followers):
	...
	try:
		...
		time.sleep(60*15)
		fo_list=getFollowers(api, root_user, no_of_followers)
	print (fo_list)
	return fo_list
...
def getSecondaryFollowers(api, followers_list, no_of_followers):
	...
	for user in followers_list:
		try:
			for user1 in tweepy.Cursor(api.followers,screen_name=user[0]).items(no_of_followers):		
				fof.append((user1.screen_name,user[0]))
		except tweepy.error.RateLimitError:
			time.sleep(60*15)
			fof=getSecondaryFollowers(api, followers_list, no_of_followers)
	print (fof)	
	return fof
...
def getFriends(api, root_user, no_of_friends):
	...
	try:
		for user in tweepy.Cursor(api.friends,screen_name=root_user).items(no_of_friends):		
			...
			time.sleep(60*15)
			friends_list=getFriends(api, root_user, no_of_friends)
	print (freinds_list)
	return freinds_list
...
def getSecondaryFriends(api, friends_list, no_of_friends):
	...
	for user in friends_list:
		try:
			for user1 in tweepy.Cursor(api.friends,screen_name=user[1]).items(no_of_friends):		
				ff.append((user[1],user1.screen_name))
		except tweepy.error.RateLimitError:
			time.sleep(60*15)
			ff=getSecondaryFriends(api, friends_list, no_of_friends)
	print (ff)
	return ff
